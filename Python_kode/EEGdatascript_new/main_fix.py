...
def Main_Algorithm(Y, M, L, n_seg, A_real, L_covseg = 10, fix_A = 'CovDL'): 
    """
    input:
    ...
    #################################  M-SBL  #####################################
        ...
        if fix_A == 'real':
            print('bruger real')
            X_rec = M_SBL.M_SBL(A_real, Y[i], M, N, k, iterations=1000, noise=False)
            
        else: 
            raise SystemExit('fix_A is not properly defined')
