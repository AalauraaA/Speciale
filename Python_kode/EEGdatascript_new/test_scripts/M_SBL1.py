...
def M_SBL(A, Y, m, n, non_zero, iterations, noise):
    ...
    if noise is False:
        ...
        Y_var = np.var(Y)
        k = 0
        while any(gamma[k] >= 10E-16):
            Gamma[k] = np.diag(np.reshape(gamma[k],(n)))        # size 1 x 1?
            for i in range(n):
                " Making Sigma and Mu "
                Sigma_y =  (np.identity(m)*Y_var)+(np.linalg.multi_dot([A, Gamma[k], A.T]))
                Sigma[k] = (Gamma[k]) - (np.linalg.multi_dot([Gamma[k],A.T,np.linalg.pinv(Sigma_y),A,Gamma[k]]))
                        
                mean[k] = np.linalg.multi_dot([Gamma[k],A.T,np.linalg.pinv(Sigma_y),Y])
...
#    print('H shape is {}'.format(gamma.shape))
    print(gamma[0],gamma[3],gamma[4],gamma[5])
