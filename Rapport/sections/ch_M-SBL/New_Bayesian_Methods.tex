...
\end{align}
with measurement matrix $\mathbf{Y} \in \mathbb{R}^{M \times L}$, sources matrix $\mathbf{X} \in \mathbb{R}^{N \times L}$ mixing matrix $\mathbf{A} \in \mathbb{R}^{M \times N}$ and noise matrix $\mathbf{E} \in \mathbb{R}^{M \times L}$. Note that $\textbf{A}$ is known throughout the chapter, as it was found by Cov-DL in chapter \ref{ch:Cov-DL}.
...
Assume the likelihood function $p(\mathbf{Y} \vert \mathbf{X})$ is Gaussian, with known noise variance $\sigma^2$\todo{skal dette begrundes? det stammer fra det ordiginale optimeringsproblem recast in bayesian terms (I.10) i pdf}. 
...
\end{align*}
\todo{note one whole row is controlled by the same hyperparameter, leading to the same support over time}
