...
From M-SBL the number of active sources $k$ is estimated as the number of non-zero entries in the hyperparameter $\boldsymbol{\gamma}$.
...
With $k$ being known the estimation of the support set $S$ from the non-zero rows of $\boldsymbol{\gamma}$, cf. section \ref{seg:EBE}, is overruled.
Instead, when generating the support set $S$ one choose the $k$ largest entries of $\boldsymbol{\gamma}$.\todo{Jeg kan ikke få det sidste til at give mening her}
 such that \cite[p. 3]{Balkan2014}
\begin{align*}
...
\For{$j = 1, \dots, k$}	
	\If{$\boldsymbol{\gamma} (\arg \max (\boldsymbol{\gamma}))$ != $0$}
		...
		\State $\boldsymbol{\gamma}(\arg \max (\boldsymbol{\gamma}_{\text{value}})) = 0$
	\EndIf
\EndFor
