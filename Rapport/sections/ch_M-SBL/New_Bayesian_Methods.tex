\todo{Husk: Skriv om hvorfor nu skriver ud fra bayesian teori}.
Consider the multiple measurement vector model (MMV)
\begin{align*}
\mathbf{Y} = \mathbf{AX} + \mathbf{E},
\end{align*}
where the unknown mixing matrix $\mathbf{A} \in \mathbb{R}^{M \times N}$ and source matrix $\mathbf{X} \in \mathbb{R}^{N \times L}$ are wished recovered from the known EEG measurement matrix $\mathbf{Y} \in \mathbb{R}^{M \times L}$ in the case of $M < N$. 
In chapter \ref{ch:Cov-DL} the mixing matrix $\mathbf{A}$ was found. 
In this chapter, a method to recover the source matrix $\mathbf{X}$ is sought. 
The approach is to find the support set $S$ of $\textbf{X}$ providing the non-zeros rows of $\mathbf{X}$ which corresponds to localization of the active sources. 
By ... the active sources are identified in order to fully recover $\textbf{X}$.
...
Within this Bayesian framework the source matrix $\mathbf{X}$ is seen as a variable which is drawn from some distribution $p(\mathbf{X})$ such that it is possible to narrow down the infinitely solution space. 
...
The fixed point updating step is achieved by taking the derivative of the marginal log likelihood $\mathcal{l}(\boldsymbol{\gamma})$ with respect to $\boldsymbol{\gamma}$ and equating it with zero. 
