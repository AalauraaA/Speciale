...
\begin{enumerate}
\item The active sources $\mathbf{X}_{S^k}$ are orthogonal. That is, $\mathbf{X}_S \mathbf{X}_{S^k}^T = \boldsymbol{\Lambda}$, where $\boldsymbol{\Lambda}$ is a diagonal matrix.
