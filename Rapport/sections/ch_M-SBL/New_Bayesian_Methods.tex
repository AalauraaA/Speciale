...
\end{align}
\todo{den del vi mangler ift. kilden er måske at opstille det optimering problem vi kender for X, hvor sigma indgår, det finder man så ud af at vi kan erstatte men MAP estimerings problemet.}The model consist of the measurement matrix $\mathbf{Y} \in \mathbb{R}^{M \times L}$, source matrix $\mathbf{X} \in \mathbb{R}^{N \times L}$, mixing matrix $\mathbf{A} \in \mathbb{R}^{M \times N}$ and noise matrix $\mathbf{E} \in \mathbb{R}^{M \times L}$. Note that $\mathbf{A}$ is known throughout the chapter, as it is found by COV-DL in chapter \ref{ch:Cov-DL}.
...
The formal framework of Bayesian statistics is Bayes' theorem\todo{kilde på bayes theorem}. The objective of Bayes' theorem is the leverage of both data and some specified prior. This is where the distinguishes from the likelihood of classical frequentist statistics lies.     
...
That is the entries of $\mathbf{X}$ is drawn from some distribution which has a sharp, possibly infinite, spike at zero surrounded by fat tails. Here the fat tails make room for the few non-zero values, which can be seen as outliers.\todo{theoretical example of t-distribution have been left out here}  
%remenber that an theoretical example could be a multivariat t-distribution with med få frihedsgrader, og \chi i anden fordeling som variance 'Rasmus'.   
...
As mentioned, the case where one would like to find the noise free sparse representations should be considered. That is the limit when $\sigma^2\rightarrow 0$. Here the M-SBL steps can be adapted easily by using a modified version the the moments , given by \cite[p.148]{phd_wipf} as 
\begin{align*}
...
\todo[inline]{this section has to be written better INDSÆT her kilde til M-SBL hvor de siger de også tager de k største værdier i gamma.}
In this algorithm the number of active sources $k$ is estimated as the non-zero entries of the hyper parameter $\boldsymbol{\gamma}$. However k has to be determined prior to the use of the M-SBL algorithm as it is used in the COV-DL algorithm. Thus there have been no reason to not pass the value of $k$ on to this algorithm. 
...
\todo{algorithm has to be updated maybe simplified at little, obs. on the while expression, number of iterations and whether k is given og not}
\begin{algorithm}[H]
\caption{M-SBL}
\begin{algorithmic}[1]
