...
%%%
By Bayes' theorem the posterior probability of source matrix $\textbf{X}$ given the measurements $\textbf{Y}$ is defined by
\begin{align*}
\textbf{p}(\textbf{X}|\textbf{Y})=\frac{\textbf{p}(\textbf{Y}|\textbf{X})\textbf{p}(X)}{\textbf{p}()}
\end{align*}  
With the knowledge of $\mathbf{A}$ and $\mathbf{Y}$ it is possible to find the maximum of the posterior distribution referred to as maximum a posteriori (MAP). That is the sources $\textbf{X}$ with the highest probability of causing the given measurements. 
The MAP estimate is closely related to the  maximum likelihood estimate..  
