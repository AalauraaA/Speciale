...
The formal framework of Bayesian statistics is Bayes' theorem. The objective of Bayes' theorem is the leverage of both data and some specified prior. This is where the distinguishes from the likelihood of classical frequentist statistics lies.     
...
That is $\mathbf{X}$ is drawn from some distribution which has a sharp, possibly infinite, spike at zero surrounded by fat tails \todo{Jan spørger efter hvorfor vi gerne vil have fat tails? - L}.
...
Assume the likelihood function $p(\mathbf{Y} \vert \mathbf{X})$ is Gaussian, with known noise variance $\sigma^2$ \todo{Jan spørger om dette en kendt kovarians matrix, men i dette tilfælde er det vel bare en varians - L}.
Due to $\textbf{Y}$ containing samples from multiple sensors over time each entry of $\textbf{Y}$ are independent and equally distributed with likelihood \todo{Jan mangler belæg på hvorfor dette er sandt - L}
\begin{align*}
...
This lead to the following update rule which can replace the above M-step in the EM-algorithm \todo{Indsæt kilde - L}
\begin{align}
...
In practise some arbitrary small threshold can be used such that that any sufficiently small hyperparameter is discarded \todo{Jan vil have en kilde her - L}.
