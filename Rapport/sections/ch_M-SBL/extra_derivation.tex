...
\end{bmatrix}.
\end{align*}
Then, from \cite{conditional_cov} \todo{Hvad er dette for en kilde?}, the posterior distribution of $p\left( \mathbf{x}_{\cdot j} \vert \mathbf{y}_{\cdot j} ; \boldsymbol{\gamma} \right)$ is defined as\todo{For $\boldsymbol{\Sigma}$ er det $\boldsymbol{\Sigma}_{\mathbf{yy}}^{-1}$ eller $\boldsymbol{\Sigma}_{\mathbf{yy \vert \mathbf{x}}}^{-1}$. Vi introducere den sidste sigma men kan ikke finde nogen steder for vi anvender den}
\begin{align*}
