...
\end{bmatrix}.
\end{align*}
Then, from \cite{conditional_cov} \todo{Hvad er dette for en kilde?}, the posterior distribution of $p\left( \mathbf{x}_{\cdot j} \vert \mathbf{y}_{\cdot j} ; \boldsymbol{\gamma} \right)$ is defined but the covariance for $\mathbf{y}$ is not known. Instead, one assume that the covariance for $\mathbf{y}$ given $\mathbf{x}$ can be used instead:
\begin{align*}
