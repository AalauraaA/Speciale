...
\\
For rank-deficient matrices there do not exist an inverse and therefore \eqref{eq:SMV_model} can not be solve by inverting the model. But rank-deficient matrices  meaning have non-empty null space leading to infinitely solutions to \eqref{eq:SMV_model} \cite[p. ix]{CS}\todo{hvad siger vi her}.
\\ \\
...
To find a solution to the linear model \eqref{eq:SMV_model} which is $k$-sparse with $k < M$ an optimisation problem can be used. An optimisation problem is defined as\todo{mangler vi ikke at skrive hvorfor vi skal løse nummerisk og ikke analytisk. er det ikke bare at der i k-sparse tilfælde kun findes en nummerisk løsning?}
\begin{align*}
