...
Compressive sensing is the theory of efficient recovery or reconstruction of a signal from a minimal number of observed measurements. Assume linear acquisition of the original information, then the relation between the measurements and the signal to be recovered can be described by the linear model giving by \eqref{eq:SMV_model} \cite{FR}.  
\\ 
...
In compressive sensing terminology, $\mathbf{x}$ is the signal of interest which is sought recovered by solving the linear system \eqref{eq:SMV_model}. In the typical compressive sensing case the system is under-determined, $M < N$,  and there exist infinitely many solutions, provided that a solution exist.
...
To find a solution to the linear model \eqref{eq:SMV_model} assuming the solution is $k$-sparse with $k < M$\todo{første gang vi møder k<M, forklaring?}, it can be view as an optimisation problem. An optimisation problem is defined as
\begin{align*}
...
\\
To find the $k$-sparse solution our optimisation problem can be written as\todo{jeg har ædret objektiv funktionen her, fra $\min_{\Omega}$}
\begin{align*}
\textbf{x}^\ast = \arg \min_{\textbf{x}\in \mathbb{R}^{N}} \Vert \mathbf{x} \Vert_0 \quad \text{s.t} \quad \mathbf{y} = \mathbf{Ax},
\end{align*}
...
\textbf{x}^\ast = \arg \min_{\textbf{x}\in \mathbb{R}^{N}} \Vert \mathbf{x} \Vert_1 \quad \text{s.t} \quad \mathbf{y} = \mathbf{Ax}.
\end{align} 
...
\begin{theorem}
A coefficient matrix $\mathbf{A} \in \mathbb{R}^{M \times N}$ is defined with columns $\textbf{A} = [\textbf{a}_1, \dots, \textbf{a}_N]$. By assuming uniqueness of a solution $\textbf{x}^{\ast}$ to
$$
...
\end{proof}
The Basis Pursuit makes the foundation of several algorithms solving alternative versions of \eqref{eq:SMV_p1} where noise is incorporated. A different type of solution method includes a greedy algorithm such as the Orthogonal Matching Pursuit\todo{er OMP ikke en basis pursuit metode} \cite[P. 65]{FR}.    
\begin{algorithm}[H]
