...
The main attribute of ICA is the assumption that the sources in $\mathbf{x}$ are statistically independent and non-Gaussian distributed, hence the name independent components \todo{when we assume independence it is enough to solve system, why?}.
\\     
...
\todo{herover er ukommenteret et afsnit jeg ikke forstår}
...
Consider again the ICA model of a single measurement vector $\textbf{y}=\textbf{Ax}$ where the independent components can be estimated by the inverted model $\textbf{x}=\textbf{A}^{-1}\textbf{y}$. \todo{Herfra og ned til kurtosis skal lige tjekkes i gennem og rettes til} 
Let $\textbf{A}^{-1}=\textbf{B}$, now a single independent component can be seen as the linear combination 
\begin{align}
x_i = \textbf{b}_{i \bullet} \textbf{y} = \sum_k b_k y_k \label{eq:ICA_comp}
\end{align} 
where $\textbf{b}_{i \bullet}$ is the $i$-th row of $\textbf{B}$. 
The issue is now to determine $\textbf{b}_{j}$ (the $j$-th column) such that it equals the $i$-th row from the inverse $\textbf{A}$. 
As $\textbf{A}$ is unknown it is not possible to determine $\textbf{b}_{j}$ exactly, but an estimate can be found to make a good approximation. Rewriting \eqref{eq:ICA_comp} 
\begin{align*}
x_i = \textbf{b}_{i \bullet} \textbf{y} = \textbf{b}_{i \bullet} \textbf{Ax} = \textbf{q}^T \textbf{x} = \sum_{k=1} q_k x_k
\end{align*}
...
Then, since $\textbf{q}^T \textbf{x} = \textbf{b}_{i \bullet} \textbf{y}$, it is possible to vary the coefficients in $\textbf{b}$ and look at the distribution of $\textbf{b}_{i \bullet} \textbf{y}$. 
...
Thus maximizing the non-Gaussianity of $\textbf{b}_{i \bullet} \textbf{y}$ results in one of the independent components \cite[p. 166]{ICA}. 
\\
...
For $y$ with zero mean and unit variance, kurtosis reduces to \todo{uddyb? og tilføj kilde til kurtosis}
\begin{align*}
...
\\ \\      
Consider again the vector $\textbf{q} = \textbf{A}^T\textbf{b}$ such that $\textbf{b}_{i \bullet} \textbf{y} = \sum_{k=1} q_k x_k$. 
By the additive property of kurtosis
\begin{align*}
\text{kurt} \left( \textbf{b}_{i \bullet} \textbf{y} \right) = \sum_{k=1}q_k^4 \text{kurt}(x_k).
\end{align*}
Then the assumption of the independent components having unit variance results in $\mathbb{E}[x_i^2]= \sum_{k=1}q_k^2=1$\todo{hvordan kommer dette frem?}. 
That is geometrically that $\textbf{q}$ is constrained to the unit sphere, $\| \textbf{q}\|^2 = 1$. 
By this the optimisation problem of maximising the kurtosis of $\textbf{b}_{i \bullet} \textbf{y}$ is similar to maximizing $\vert \text{kurt}(x_i)\vert = \vert \sum_{k=1}q_k^4 \text{kurt}(x_k) \vert $ on the unit sphere.
\\
...
In practise, to recover the mixing matrix $\textbf{A}$ by maximizing the kurtosis of $\textbf{b}_{i \bullet} \textbf{y}$, gradient optimisation methods are used.
\\
The general idea behind a gradient algorithm is to determine the direction for which $\text{kurt}(\textbf{b}_{i \bullet} \textbf{y})$ is growing the most, based on the gradient. 
\\
The gradient of $\vert \text{kurt}(\textbf{b}_{i \bullet} \textbf{y}) \vert$ is computed as
\begin{align}\label{eq:kurt}
\frac{\partial \vert \text{kurt}(\textbf{b}_{i \bullet} \textbf{y})\vert}{\partial \mathbf{b}_j} &= 4 \text{sign}(\text{kurt}(\textbf{b}_{i \bullet} \textbf{y})) (\mathbb{E}[\mathbf{y} (\textbf{b}_{i \bullet} \textbf{y})^3] - 3 \mathbf{y} \mathbb{E}[(\textbf{b}_{i \bullet} \textbf{y})^2]) 
\end{align} 
As $\mathbb{E}[(\textbf{b}_{i \bullet}^T \textbf{y})^2] =\Vert \mathbf{y} \Vert^2$ for whitened data the corresponding term does only affect the norm of $\textbf{b}_j$ within the gradient algorithm. 
...
\begin{align*}
\Delta &\textbf{b}_j \propto \text{sign}\left( \text{kurt}(\textbf{b}_{i \bullet} \textbf{y}) \right) \mathbb{E}[\textbf{y}(\textbf{b}_{i \bullet}^T \textbf{y})^3] \\
...
\begin{align*}
\Delta \gamma \propto((\textbf{b}_{i \bullet} \textbf{y})^4 - 3) - \gamma
\end{align*}
...
\begin{algorithmic}[1]
            ...
            \For{$j \gets 1,2, \hdots ,N$ }
            	...
            	\While{$\text{convergence critia not meet}$} 
               		\State $k = k+1$
                	\State $\textbf{b}_{j(k)} \gets \text{sign}\gamma_{(k-1)} \textbf{y}(\textbf{b}_{i \bullet} \textbf{y})^3$
                	\State $\textbf{b}_{j(k)} \gets \textbf{b}_j/\Vert \textbf{b}_j \Vert $ 
                	\State $\gamma_{(k)} \gets ((\textbf{b}_{i \bullet} \textbf{y})^4 - 3) - \gamma_{(k-1)} $
          		\EndWhile
          		\State $x_{j} = \textbf{b}_{i \bullet}\textbf{y}$
          	\EndFor
...
\end{align*}
where $\mathbf{A}_S$ is derived from the dictionary matrix $\mathbf{A}$ by containing only the columns associated with the non-zero entries of $\textbf{x}$, specified by the support set $S$\todo{henvis til appendix}. 
