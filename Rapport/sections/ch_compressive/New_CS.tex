...
Remember from chapter \ref{ch:motivation} that EEG measurements basically are mixture of original brain signals affected by volume conduction and noise \todo{Jan's note med noise. Tjek om dette nævnes i kapitel 1 - L}.
...
The projection matrix $\mathbf{A}$ \todo{Projection/coefficient? Dette er første gang projection anvendes - L}, referred to as the mixing matrix, models the volume conduction by mapping the source vector from $\mathbb{R}^N$ to $\mathbb{R}^M$.
...
To count the non-zero row of a matrix the $\ell_0$-norm is defined \todo{Jan spørger ud fra denne ligning om vi tæller elementerne eller rækkerne - L}
\begin{align*}
...
However, this method only apply to the process of learning $\mathbf{A}$ \todo{as the relation between $\textbf{X}$ and the corresponding $\delta$ is unknown(?) det kan vi ikke skrive her kan vi?}, hence a different approach is necessary to recover $\mathbf{X}$.
...
From the log-likelihood an estimate for the source matrix $\mathbf{X}$ is obtained with respect to the support set $S$ created from the optimised variance \todo{evt. tilføj afslutning der siger hvad der kommer i kapitlerne?} \todo{De sidste to linje synes Rasmus er uklar og knudret -L}.
