...
\end{align} 
The optimal solution to \eqref{eq:SVD3} is known to be the rank-1 approximation of $\textbf{E}_{j_{0}}$. This comes from the Eckart–Young–Mirsky theorem \cite{?}\todo{Find cite} saying that a partial single value decomposition (SVD) makes the best low-rank approximation of a matrix such as $\textbf{E}_{j_0}$. The SVD is given as
\begin{align*}
...
Before PCA is applied to a dataset, the data must undergo a standardization/scaling to remove any difference in the data. 
...
With $\tilde{\mathbf{Z}}$ a correlation matrix is computed as
\begin{align*}
\boldsymbol{\Sigma}_{\tilde{\mathbf{Z}}} = \text{Corr}(\tilde{\mathbf{Z}}) = \frac{1}{m} \tilde{\mathbf{Z}}^T \tilde{\mathbf{Z}}
\end{align*}
From the correlation matrix a orthonormal basis of eigenvectors $\mathbf{p}_1, \dots, \mathbf{p}_m$ with corresponding eigenvalues $\lambda_1, \dots, \lambda_m$ exists, cf. theorem 6.15 in \cite[p.]{} \todo{indsæt cite}. Furthermore, one assume that $\lambda_1 \geq \dots \geq \lambda_m$. 
%It exist beacuse Corr is symmetric
The eigenvectors and eigenvalues can be computed from the correlation matrix by e.g. using a singular value decomposition (SVD). With SVD an orthogonal matrix $\mathbf{P}$ with the eigenvectors $\mathbf{p}_1, \dots, \mathbf{p}_m$ as columns is obtain with the associated eigenvalues as a diagonal matrix denoted as $\mathbf{D}$.
The principal components is then defined by
\begin{align*}
\mathbf{u}_i = \tilde{\mathbf{Z}} \mathbf{p}_i,
\end{align*}
where $\mathbf{p}_i$ is the $i$-th eigenvector of the correlation matrix $\boldsymbol{\Sigma}_{\textbf{Z}}$. 
Thus each principal component is a linear combination of the scaled dataset $\tilde{\mathbf{Z}}$ \cite{}.
