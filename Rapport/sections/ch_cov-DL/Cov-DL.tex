...
\text{s.t.} \ \textbf{d}_i&=\text{vec}(\textbf{a}_i\textbf{a}_i^T)
\end{align*}      
where $\textbf{U}$ is learned by use of PCA performed on $\text{vec}(\boldsymbol{\Sigma}_{\textbf{Y}_s})$.\\ 
To solve this optimization problem the gradient of the cost function is computed with respect to $\textbf{A}$ by use of the chain rule \\
... \\
then the cost function is minimized by use of quasi-Newton optimization methods.
...
\begin{algorithmic}[1]
           \Procedure{Cov-DL}{$\textbf{Y}_s$}    
			...
			\State$\textbf{T} = \text{vec}^{-1}(d_i)$            
			\State$\lambda_i\gets \max\{eigenvalue(\textbf{T})\}$
			\State$\textbf{b}_i \gets \ eigenvector(\lambda_i)$
			\State$\textbf{a}_i \gets \sqrt{\lambda_i}\textbf{b}_i$
			\EndFor
			\State$\textbf{A} = \{\textbf{a}_i\}_{i=1}^N$
			\EndIf
			\State
			\If{$N < \frac{M(M+1)}{2}$}
			\State$...$
