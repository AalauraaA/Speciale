\subsection{Under-determined system}
...
For a known sparsity constraint $k$ the dictionary learing can be defined by the following optimisation problem. 
...
\end{align}
The problem is similar to the $\ell_1$ optimisation problem defined in \eqref{eq:SMV_p1}\cite{Elad_book}, except that here both $\textbf{A}$ and $\textbf{x}_j$ are variables to be determined.  
The learning constitute joint solving of the optimization problem with respect to respectively $\mathbf{A}$ and $\mathbf{X}$. The uniqueness of $\mathbf{A}$ depends on the recovery sparsity condition. As clarified earlier in \ref{sec:sol_met} the recovery of a unique solution $\mathbf{X}^\ast$ is only possible if $k < M$ \cite{phd2015}.
...
\todo{yderligere beskrivelse af opdaterings regler}
