...
Covariance-domain dictionary learning (Cov-DL) is an algorithm proposed by O. Balkan \cite{Balkan2015}, claiming to successfully identify more active sources $k$ than available measurements $M$ from the segmented multiple measurement vector model 
\begin{align*}
\mathbf{Y}_s = \mathbf{A}_s \mathbf{X}_s + \textbf{E}_s.
\end{align*}
where $\mathbf{Y}_s \in \mathbb{R}^{M \times t_s f}$ is the observed segmented EEG measurement matrix, $\mathbf{X}_s \in \mathbb{R}^{N \times t_s f}$ the segmented source matrix and $\mathbf{E}_s \in \mathbb{R}^{M \times t_s f}$ is the additional segmented noise matrix.
\\
...
\subsection{Compressive Sensing}\label{sec:CS}
