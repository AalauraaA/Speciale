...
\end{align}
From \eqref{eq:noise1} to \eqref{eq:noise2} all terms where noise, $\boldsymbol{\epsilon}$ and $\mathbf{E}$, is included, are aggregated in a joint noise term $\widetilde{\textbf{E}}$. 
...
The goal is now to learn first $\textbf{D}$ and then the associated mixing matrix $\textbf{A}$ \todo{Hvad A ikke en dictionary matrix i dette afsnit? - L}. 
