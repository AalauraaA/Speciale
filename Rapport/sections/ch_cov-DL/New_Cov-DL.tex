...
The goal is now to learn first $\textbf{D}$ and then the associated mixing matrix $\textbf{A}$. Two methods are considered relying on the relation of $M$ and $N$. For now the noise vector is ignored\todo{update algoritme så vi ikke har newton i DL2}.   
