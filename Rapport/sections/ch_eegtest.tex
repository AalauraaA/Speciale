...
Segments at $t = 1$ second is chosen from the belief that the brain activity can be assumed stationary with in short time interval furthermore one must take in mind that shorter time interval lead to more segments and therefore a higher computational complexity\todo{se udkommentering, hvordan har nul tolerencen noget med stationarity at gøre, sætter vi ikke først rækkerne nul efter ICA?}.   
%%%
...
At the end the resulting two estimates have the same dimensions which allows for $\hat{\textbf{X}}_{Main s}$ to be evaluated with respect to $\hat{\textbf{X}}_{ICA s}$ by the MSE\todo{note here if a fitting is applied!}.             
% 
