...
With known $\mathbf{A}$ and $\mathbf{X}$ the measurement matrix $\mathbf{Y} \in \mathbb{R}^{M \times L}$ is simulated according to the model, by the matrix product $\mathbf{Y} = \mathbf{AX}$. Note that the error matrix $\mathbf{E}$ is omitted in this chapter as the focus will be to evalute the performance of the recovering \todo{Tjek lige denne sætning i forhold til fejlledet}.  
...
The first is specified by $N = 5$, $k = 4$, $M = 3$ and $L = 1000$. That is a source matrix $\mathbf{X}$ with $4$ independent \todo{Skal vi skrive different her eller beholde independent?} signals and $1$ zero row which is mixed into a measurement matrix with $3$ measurement per sample.  
...
\end{itemize}
\todo{Tænker ikke at vi behøves at indsætte cov(X,X) men bare huske det til eksamen} with $t$ being a time index defined in the interval $[0,4]$ with $L$ samples. 
