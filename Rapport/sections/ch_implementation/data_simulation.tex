...
As such the non-zero rows of $\mathbf{X}$ are mutually orthogonal\todo{Kan ikke finde noget på at independent rows af matrix leder til at den også er ortgonal. En matrix er ortogonal hvis $X * X^T = I$ eller $X^T = X^{-1}$}, which fulfils the first conditions of theorem \ref{th:conditions}.   
...
The four independent source signals of $\mathbf{X}$ are defined by 
\begin{itemize}
