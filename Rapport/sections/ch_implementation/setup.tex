...
First the measurement matrix $\mathbf{Y}_s$ for all segments are transformed to the covariance domain and then vectorized, resulting in an extension of the dimensionality from $M$ to $\widetilde{M} = \frac{M(M+1)}{2}$\todo{should we define new number of "samples" as well?}.
