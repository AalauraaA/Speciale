...
That is the algorithm visualized in the flow diagram \ref{fig:flow} where the Cov-DL algorithm and the M-SBL algorithm are combined. 
However, as discussed due to the negative conclusion on the verification of Cov-DL an alternative to the mixing matrix has to be considered.  
...
The average $\text{MSE}(\mathbf{X}, \hat{\mathbf{X}})$ are visualized in figure \ref{fig:varyN1}.
\begin{figure}[H]
    \centering
	\includegraphics[scale=0.5]{figures/ch_6/varyN1.png}
	\caption{Visualization of $\text{MSE}(\mathbf{X}, \hat{\mathbf{X}})$ of the main algorithm with simulated stochastic data sets specified by $M = 8$, $L = 1000$ and $k = N$ for $N = M+1, \dots , 36$. Average over ten repetitions for each $N$.}
	\label{fig:varyN1}
\end{figure}
\noindent
From figure \ref{fig:varyN1} it is seen that the $\text{MSE}(\mathbf{X}, \hat{\mathbf{X}})$ lies in the interval $[4,14]$.
However, no clear trend appears in the figure. 
This suggests that it is not representative averages which have been visualized. 
Thus, the test is repeated with more repetitions for each value of $N$. 
...
\begin{figure}[H]
	...
	\caption{Visualization of $\text{MSE}(\mathbf{X}, \hat{\mathbf{X}})$ of the main algorithm with simulated stochastic data sets specified by $M = 8$, $L = 1000$ and $k = N$ for $N = M+1, \dots ,36$. Average over 500 repetitions for each $N$.}
	\label{fig:varyN2}
\end{figure}  
\noindent
...
To investigate this behaviour the corresponding box-plot presenting the 500 repetitions is visualized in figure \ref{fig:box}, respectively with and without outliers. 
...
Thus, the choice of the alternative estimate $\hat{\mathbf{A}}_{\text{norm2}}$ might have influenced the results negatively. 
