...
This suggest some flaw\todo{Jan sættet spørgsmål ved dette ord, jeg ved ikke hvorfor - L} within the optimization process. 
...
This suggest that the optimization problem, derived in section \ref{sec:over_det}, do not fulfil the purpose.\todo{better word to use here?} \todo{Jan spørger om vi ved hvorfor den ikke opfylder. Måske det er fordi den er convex? - L}
...
The appearance of this issue may suggest that there is a lack within the published results \cite{Balkan2015} \todo{(tjek cite)} considering the possibility of reproducibility\todo{tjek sætning om den lyder rigtig efter "reproducibility" er sat ind -L} of the results.  
...
From this is can be argued that letting $N = k$ is as good an estimate of $N$ as any other. Note in order to fulfil the sufficient conditions for the existence of a solution to the system, that $k$ hence $N$ is limited by $\widetilde{M}$ \todo{Jan mener at denne sætning er svagt formuleret, "as any other" - L}.     
