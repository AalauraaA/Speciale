...
\begin{align*}
\hat{\mathbf{A}} = \text{argmin}\  \text{cost}(\mathbf{A}) \leq \text{cost}(\mathbf{A}) \geq \text{cost}(\hat{\mathbf{A}}) \leq \text{cost}(\mathbf{A})\quad \forall \mathbf{A} 
\end{align*}  
...
It appears to that the issue of the model being invariant towards the order of the rows in the estimate $\hat{\mathbf{A}}$ is not an issue in this example. 
...
Hence, it will not be used as an input for the next stage of the main algorithm, M-SBL. 
...
From the flowchart \ref{fig:flow} it seen that the M-SBL algorithm takes estimate of the mixing matrix $\hat{\mathbf{A}}$ and measurement matrix $\mathbf{Y}$ as input.
...
\caption{Estimated values of $\hat{\mathbf{X}}$ compared to the true values $\mathbf{X}$. From deterministic data set specified by $N = k = 4$, $M = 3$ and $L=1000$ with the true mixing matrix $\mathbf{A}$.}
...
\end{align*}
From figure \ref{fig:M-SBL_simple1} it is seen that all four source signals are recovered at the right locations relative to the zero rows removed from $\mathbf{X}$ and $\hat{\mathbf{X}}$. 
...
\end{align*}
From figure \ref{fig:M-SBL_simple2} it is again seen that the source signals are recovered at the right locations relative to the locations of the non-zero rows after removal of zero rows. 
