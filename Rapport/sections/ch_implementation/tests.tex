...
From the problem statement in chapter \ref{ch:problemstatement} it is an issue that $k$ has to be known a priori, in order to estimate $\textbf{A}$ and $\textbf{X}$. A short discussion in subsection \ref{subsec:kestimate}, describes how k can be estimated within the M-SBL algorithm. However, one still needs to provide k in order to estimate $\textbf{A}$, thus a qualified estimate of $k$ can not be avoided. 
