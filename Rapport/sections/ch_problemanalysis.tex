...
\todo{Nyt afsnit, læs} For the purpose of recovering $\mathbf{x}$, from $\mathbf{y}$ and $\mathbf{A}$, a multiple measurement sparse Bayesian learning (M-SBL) algorithm is proposed. M-SBL is based on the concept of finding a set of non-zeros indices of the source vector $\mathbf{x}$ which corresponds to finding the localisation of sources. This method is also targeting the case of more active sources than sensors.
